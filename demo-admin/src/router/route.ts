...
/**
 * 路由path路径与文件夹名称相同，找文件可以通过浏览器地址快速查找，方便定位文件
 *
 * 路由meta对象参数，我们通常将属性放到meta对象中
 * meta: {
 *   title:     菜单栏以及tabsView 栏、菜单搜索名称（国际化）
 *   status:    菜单状态：是否隐藏此路由，不会显示在菜单树，可以访问
 *   disable:   是否停用，不会显示在菜单树，且不可访问
 *   keepAlive: 是否缓存组件状态？
 *   affix:     是否固定在tabsView 栏上
 *   link:      是否是超链接菜单，开启外链条件：1、link：链接地址不为空  2、iframe: false
 *   iframe:    是否内嵌窗口，开启条件：1、iframe：true  2、link：链接地址不为空
 *   roles:     当前路由权限表示，取角色管理。路由控制显示、隐藏、超管管理员：admin；普通角色：common
 *   icon:      菜单、tabsView 图标码
 *   svgIcon:   svg图标
 *   sort:      菜单顺序
 * }
 */
 ...
 * @description 前端控制路由 直接取mock/_data/system_menu 中的路由，后端控制则不需要
 * @returns 返回路由菜单数据
 */
export const staticRoutes = [
  ...
  /**
   * 提示：写在这里的为全屏页面，不建议在这里非全屏页面，请写在 layout.children 路由数组中
   *
   */
];
